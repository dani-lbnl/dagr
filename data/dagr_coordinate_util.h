#ifndef dagr_cartesian_mesh_util_h
#define dagr_cartesian_mesh_util_h
    ...
    data_t val, bool lower, unsigned long &id)
{
    ...
    if (val < data[m_0])
    {
        ...
        return dagr_coordinate_util::index_of<data_t, bracket_t>(
...
int index_of(const T *data, size_t l, size_t r, T val, unsigned long &id)
{
    ...
    {
        // split range to the left
        return dagr_coordinate_util::index_of(data, l, m_0, val, id);
    }
    else
    {
        // split the range to the right
        return dagr_coordinate_util::index_of(data, m_1, r, val, id);
    }
...
template<typename T>
int index_of(const const_p_dagr_cartesian_mesh &mesh, T x, T y, T z,
        unsigned long &i, unsigned long &j, unsigned long &k)
{
    const_p_dagr_variant_array xc = mesh->get_x_coordinates();
    const_p_dagr_variant_array yc = mesh->get_y_coordinates();
    const_p_dagr_variant_array zc = mesh->get_z_coordinates();
    ...
    TEMPLATE_DISPATCH_FP(
        ...
        if (dagr_coordinate_util::index_of(p_xc, 0, nx-1, x, true, i)
            || dagr_coordinate_util::index_of(p_yc, 0, ny-1, y, true, j)
            || dagr_coordinate_util::index_of(p_zc, 0, nz-1, z, true, k))
        {
            // out of bounds
            return -1;
        }
