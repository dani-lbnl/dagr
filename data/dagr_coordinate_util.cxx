...
namespace dagr_coordinate_util
{

// **************************************************************************
int time_step_of(p_dagr_double_array time, bool lower,
    ...
    const std::string &date, unsigned long &step)
{
    ...
    if (n_conv < 1)
    {
        DAGR_ERROR("invalid start date \"" << date
    ...
    {
        DAGR_ERROR("failed to convert date \"" << date
    ...
    if (dagr_coordinate_util::index_of(time->get(), 0, last, t, lower, step))
    {
        DAGR_ERROR("failed to locate the requested time " << t << " in ["
    ...
    (void)date;
    step = 0;
    DAGR_ERROR("The UDUnits package is required for this operation")
    return -1;
#endif
}
...
int bounds_to_extent(const double *bounds,
    ...
    const_p_dagr_variant_array z, unsigned long *extent)
{
    TEMPLATE_DISPATCH_FP(
        ...
        if (((nx > 1) && (((px[high_i] > px[0]) &&
            (dagr_coordinate_util::index_of(px, 0, high_i, low_x, true, extent[0])
            || dagr_coordinate_util::index_of(px, 0, high_i, high_x, slice_x, extent[1]))) ||
            ((px[high_i] < px[0]) &&
            (dagr_coordinate_util::index_of<NT,descend_bracket<NT>>(px, 0, high_i, low_x, false, extent[0])
            || dagr_coordinate_util::index_of<NT,descend_bracket<NT>>(px, 0, high_i, high_x, !slice_x, extent[1])))))

            || ((ny > 1) && (((py[high_j] > py[0]) &&
            (dagr_coordinate_util::index_of(py, 0, high_j, low_y, true, extent[2])
            || dagr_coordinate_util::index_of(py, 0, high_j, high_y, slice_y, extent[3]))) ||
            ((py[high_j] < py[0]) &&
            (dagr_coordinate_util::index_of<NT,descend_bracket<NT>>(py, 0, high_j, low_y, false, extent[2])
            || dagr_coordinate_util::index_of<NT,descend_bracket<NT>>(py, 0, high_j, high_y, !slice_y, extent[3])))))

            || ((nz > 1) && (((pz[high_k] > pz[0]) &&
            (dagr_coordinate_util::index_of(pz, 0, high_k, low_z, true, extent[4])
            || dagr_coordinate_util::index_of(pz, 0, high_k, high_z, slice_z, extent[5]))) ||
            ((pz[high_k] < pz[0]) &&
            (dagr_coordinate_util::index_of<NT,descend_bracket<NT>>(pz, 0, high_k, low_z, false, extent[4])
            || dagr_coordinate_util::index_of<NT,descend_bracket<NT>>(pz, 0, high_k, high_z, !slice_z, extent[5]))))))

        {
            DAGR_ERROR(<< "requested subset [" << bounds[0] << ", " << bounds[1] << ", "
    ...
    DAGR_ERROR("invalid coordinate array type")
    return -1;
}
};
