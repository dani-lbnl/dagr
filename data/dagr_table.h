...
*/
class dagr_table : public dagr_dataset
{
public:
    DAGR_DATASET_STATIC_NEW(dagr_table)
    DAGR_DATASET_NEW_INSTANCE()
    DAGR_DATASET_NEW_COPY()

    virtual ~dagr_table() = default;
    ...
    p_dagr_variant_array get_column(unsigned int i);
    p_dagr_variant_array get_column(const std::string &col_name);

    const_p_dagr_variant_array get_column(unsigned int i) const;
    const_p_dagr_variant_array get_column(const std::string &col_name) const;
    ...
    // add a column
    int append_column(p_dagr_variant_array array)
    { return m_impl->columns->append(array); }

    int append_column(const std::string &name, p_dagr_variant_array array)
    ...
    p_dagr_array_collection get_columns()
    { return m_impl->columns; }

    const_p_dagr_array_collection get_columns() const
    { return m_impl->columns; }
    ...
    void copy(const const_p_dagr_table &other,
    ...
    void copy_structure(const const_p_dagr_table &other);
    ...
    // columns.
    void concatenate_rows(const const_p_dagr_table &other);
    ...
    void concatenate_cols(const const_p_dagr_table &other, bool deep=false);

protected:
    dagr_table();
    dagr_table(const dagr_table &other) = default;
    dagr_table(dagr_table &&other) = default;
    dagr_table &operator=(const dagr_table &other) = default;
...
private:
    struct impl_t
    {
        impl_t();
        //
        p_dagr_array_collection columns;
...
// --------------------------------------------------------------------------
inline
p_dagr_variant_array dagr_table::get_column(unsigned int i)
{
...
// --------------------------------------------------------------------------
inline
const_p_dagr_variant_array dagr_table::get_column(unsigned int i) const
{
    return m_impl->columns->get(i);
}
...
// --------------------------------------------------------------------------
template<typename nT, typename cT, typename... oT>
void dagr_table::declare_columns(nT &&col_name, cT col_type, oT &&... args)
{
...
// --------------------------------------------------------------------------
template<typename nT, typename cT>
void dagr_table::declare_column(nT &&col_name, cT col_type)
{
    m_impl->columns->declare(std::forward<nT>(col_name), col_type);
}
...
// --------------------------------------------------------------------------
template<typename cT, typename... oT>
void dagr_table::append(cT &&val, oT &&... args)
{
...
template<typename T>
p_dagr_table &operator<<(p_dagr_table &t, T &&v)
{
