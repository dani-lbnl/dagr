...
// ----------------------------------------------------------------------------
void dagr_array_collection::clear()
{
    m_arrays.clear();
    m_names.clear();
...
// ----------------------------------------------------------------------------
int dagr_array_collection::append(p_dagr_variant_array array)
{
...
// ----------------------------------------------------------------------------
int dagr_array_collection::append(
    const std::string &name,
    p_dagr_variant_array array)
{
...
// ----------------------------------------------------------------------------
int dagr_array_collection::set(unsigned int i, p_dagr_variant_array array)
{
    if (i >= m_names.size())
        return -1;
...
// ----------------------------------------------------------------------------
int dagr_array_collection::remove(const std::string &name)
{
    name_array_map_t::iterator loc = m_name_array_map.find(name);
    if (loc != m_name_array_map.end())
    {
...
// ----------------------------------------------------------------------------
int dagr_array_collection::remove(unsigned int id)
{
    m_arrays.erase(m_arrays.begin()+id);
    return 0;
}
...
// ----------------------------------------------------------------------------
bool dagr_array_collection::has(const std::string &name) const
{
    return m_name_array_map.count(name) > 0;
}

// ----------------------------------------------------------------------------
p_dagr_variant_array dagr_array_collection::get(const std::string &name)
{
...
// ----------------------------------------------------------------------------
const_p_dagr_variant_array dagr_array_collection::get(const std::string &name) const
{
    name_array_map_t::const_iterator loc = m_name_array_map.find(name);
    if (loc == m_name_array_map.cend())
        return nullptr;
...
// ----------------------------------------------------------------------------
void dagr_array_collection::copy(const const_p_dagr_array_collection &other)
{
...
// ----------------------------------------------------------------------------
void dagr_array_collection::shallow_copy(const p_dagr_array_collection &other)
{
    m_names = other->m_names;
    m_name_array_map = other->m_name_array_map;
...
// --------------------------------------------------------------------------
void dagr_array_collection::swap(p_dagr_array_collection &other)
{
...
// --------------------------------------------------------------------------
void dagr_array_collection::to_stream(dagr_binary_stream &s) const
{
...
// --------------------------------------------------------------------------
void dagr_array_collection::from_stream(dagr_binary_stream &s)
{
    unsigned int na;
    s.unpack(na);
    s.unpack(m_names);
    ...
    for (unsigned int i = 0; i < na; ++i)
    {
        ...
        m_arrays[i] = dagr_variant_array_factory::New(type_code);
...
// --------------------------------------------------------------------------
void dagr_array_collection::to_stream(std::ostream &s) const
{
    s << "{" << std::endl;
