...
#if !defined(DAGR_HAS_REGEX)
    (void)code;
    return "c++11 regex support is disabled in this build of DAGR";
#else
    switch (code)
    {
...
int set_filter_regex(const std::string &re)
{
    ...
    try
    {
       dagr_file_util::filter_regex = std::regex(re, std::regex::grep);
    }
...
#else
    std::cmatch matches;
    if (std::regex_search(de->d_name, matches, dagr_file_util::filter_regex))
        return 1;
    return 0;
#endif
}
...
{
    if (dagr_file_util::set_filter_regex(re))
        return -1;
    ...
    {
        int e = errno;
        DAGR_ERROR("Failed to scan for files" << std::endl << strerror(e))
        return -2;
    }
    else
    if (n_files < 1)
    {
        DAGR_ERROR("Found no files matching regular expression" << std::endl << re)
        return -3;
    }
...
size_t load_text(const std::string &filename, std::string &text)
{
    ...
    if (!file.is_open())
    {
        DAGR_ERROR("File " << filename << " could not be opened.")
        return 0;
    }
...
int read_stream(const char *file_name, const char *header,
    dagr_binary_stream &stream, bool verbose)
{
    // open the file
    ...
    if (fd == NULL)
    {
        if (verbose)
        {
            ...
            DAGR_ERROR("Failed to open " << file_name << ". " << estr)
        }
        return -1;
    }
    ...
    if (fread(file_header, 1, header_len, fd) != header_len)
    {
        ...
        fclose(fd);
        free(file_header);
        DAGR_ERROR("Failed to read header from \""
            << file_name << "\". " << estr)
        return -1;
    }
    ...
    if (strncmp(file_header, header, header_len))
    {
        fclose(fd);
        free(file_header);
        DAGR_ERROR("Header missmatch in \""
    ...
    if (bytes_read != nbytes)
    {
        const char *estr = (ferror(fd) ? strerror(errno) : "");
        fclose(fd);
        DAGR_ERROR("Failed to read \"" << file_name << "\". Read only "
    ...
    {
        const char *estr = strerror(errno);
        DAGR_ERROR("Failed to close \"" << file_name << "\". " << estr)
        return -1;
    }
...
int write_stream(const char *file_name, const char *header,
    const dagr_binary_stream &stream, bool verbose)
{
    // open up a file
    ...
    if (fd == -1)
    {
        ...
        {
            const char *estr = strerror(errno);
            DAGR_ERROR("Failed to create \"" << file_name << "\". " << estr)
        }
        return -1;
    }

    // this will let the reader verify that we have a dagr binary table
    long header_len = strlen(header);
    if (write(fd, header, header_len) != header_len)
    {
        const char *estr = strerror(errno);
        DAGR_ERROR("Failed to write header to \""
            << file_name << "\". " << estr)
        return -1;
    }
    ...
    while (n_to_write > 0)
    {
        ...
        {
            const char *estr = strerror(errno);
            DAGR_ERROR("Failed to write \"" << file_name << "\". " << estr)
            return -1;
        }
        n_wrote += n;
        n_to_write -= n;
