...
void write_vtk_array_data(FILE *ofile,
    const const_p_dagr_variant_array &a, int binary)
{
    if (a)
    {
        size_t na = a->size();
        TEMPLATE_DISPATCH(const dagr_variant_array_impl,
            ...
            {
                char fmt_delim[32];
                snprintf(fmt_delim, 32, " %s", dagr_vtk_util::vtk_tt<NT>::fmt());
                fprintf(ofile, dagr_vtk_util::vtk_tt<NT>::fmt(), pa[0]);
...
int write_vtk_legacy_header(FILE *ofile,
    const const_p_dagr_variant_array &x, const const_p_dagr_variant_array &y,
    const const_p_dagr_variant_array &z, bool binary,
    const std::string &comment = "")
{
    if (!x && !y && !z)
    {
        DAGR_ERROR("data must be at least 1 dimensional")
        return -1;
    }
    ...
    if (comment.empty())
    {
        ...
        fprintf(ofile, "DAGR vtk_cartesian_mesh_writer "
            DAGR_VERSION_DESCR " %s\n", date_str);
    }
    else
    {
        fprintf(ofile, "%s\n", comment.c_str());
    ...
    TEMPLATE_DISPATCH(const dagr_variant_array_impl,
        ...
        coord_type_str = dagr_vtk_util::vtk_tt<NT>::str();
        )
...
int write_vtk_legacy_attribute(FILE *ofile,
    const const_p_dagr_array_collection &data, center_t cen, bool binary)
{
    ...
    {
        const_p_dagr_variant_array array = data->get(i);
        ...
        TEMPLATE_DISPATCH(const dagr_variant_array_impl,
            ...
            dagr_vtk_util::vtk_tt<NT>::str());
            )
        else
        {
            DAGR_ERROR("unsupported type encountered")
            return -1;
        }
...
// --------------------------------------------------------------------------
dagr_vtk_cartesian_mesh_writer::dagr_vtk_cartesian_mesh_writer()
    : file_name(""), binary(0)
{
...
#if defined(DAGR_HAS_BOOST)
// --------------------------------------------------------------------------
void dagr_vtk_cartesian_mesh_writer::get_properties_description(
    const std::string &prefix, options_description &global_opts)
{
    options_description opts("Options for "
        + (prefix.empty()?"dagr_vtk_cartesian_mesh_writer":prefix));

    opts.add_options()
        DAGR_POPTS_GET(std::string, prefix,file_name,
            "path/name to write series to")
        DAGR_POPTS_GET(int, prefix,binary,
...
// --------------------------------------------------------------------------
void dagr_vtk_cartesian_mesh_writer::set_properties(
    const std::string &prefix, variables_map &opts)
{
    DAGR_POPTS_SET(opts, std::string, prefix, file_name)
    DAGR_POPTS_SET(opts, int, prefix, binary)
}
#endif

// --------------------------------------------------------------------------
const_p_dagr_dataset dagr_vtk_cartesian_mesh_writer::execute(
    unsigned int port, const std::vector<const_p_dagr_dataset> &input_data,
    const dagr_metadata &request)
{
    (void)port;

    const_p_dagr_cartesian_mesh mesh
    ...
    {
        DAGR_ERROR("request missing \"time_step\"")
        return nullptr;
    }
    ...
    {
        DAGR_ERROR("request missing \"time\"")
        return nullptr;
    }
    ...
    if (dagr_vtk_util::deep_copy(rg, mesh))
    {
        DAGR_ERROR("Failed to copy to vtkRectilinearGrid")
        return nullptr;
    }

    std::string out_file = this->file_name;
    dagr_file_util::replace_timestep(out_file, time_step);
    dagr_file_util::replace_extension(out_file, "vtr");
    ...
    if (dagr_file_util::file_exists(pvd_file_name.c_str()))
    {
        ...
        if (!pvd_file.good())
        {
            DAGR_ERROR("Failed to open \"" << pvd_file_name << "\"")
            return nullptr;
        }
    ...
    std::string out_file = this->file_name;
    dagr_file_util::replace_timestep(out_file, time_step);
    dagr_file_util::replace_extension(out_file, "vtk");
    ...
    if (!ofile)
    {
        ...
        DAGR_ERROR("Failed to open \"" << out_file << "\""
             << std::endl << err_desc)
        return nullptr;
    }
    ...
    {
        DAGR_ERROR("failed to write the header")
        return nullptr;
    }
    ...
    if (internals::write_vtk_legacy_attribute(ofile,
        ...
        this->binary))
    {
        DAGR_ERROR("failed to write point arrays")
        return nullptr;
    }
