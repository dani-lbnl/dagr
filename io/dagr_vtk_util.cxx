...
namespace dagr_vtk_util
{
// **************************************************************************
int deep_copy(vtkRectilinearGrid *output,
    const_p_dagr_cartesian_mesh input)
{
#if defined(DAGR_HAS_VTK) || defined(DAGR_HAS_PARAVIEW)
    // set the extent
    ...
    {
        DAGR_ERROR("input missing \"extent\"")
        return -1;
    }
    ...
    const_p_dagr_variant_array x = input->get_x_coordinates();
    TEMPLATE_DISPATCH(const dagr_variant_array_impl, x.get(),
    ...
    const_p_dagr_variant_array y = input->get_y_coordinates();
    TEMPLATE_DISPATCH(const dagr_variant_array_impl, y.get(),
    ...
    const_p_dagr_variant_array z = input->get_z_coordinates();
    TEMPLATE_DISPATCH( const dagr_variant_array_impl, z.get(),
    ...
    const_p_dagr_array_collection pd = input->get_point_arrays();
    unsigned int n_arrays = pd->size();
    for (unsigned int i = 0; i< n_arrays; ++i)
    {
        const_p_dagr_variant_array a = pd->get(i);
        std::string name = pd->get_name(i);

        TEMPLATE_DISPATCH(const dagr_variant_array_impl, a.get(),
