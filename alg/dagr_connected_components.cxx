...
// --------------------------------------------------------------------------
dagr_connected_components::dagr_connected_components() :
...
// --------------------------------------------------------------------------
std::string dagr_connected_components::get_label_variable(
    const dagr_metadata &request)
{
    std::string label_var = this->label_variable;
    if (label_var.empty())
    {
        if (request.has("dagr_connected_components::label_variable"))
            request.get("dagr_connected_components::label_variable", label_var);
...
// --------------------------------------------------------------------------
std::string dagr_connected_components::get_threshold_variable(
    const dagr_metadata &request)
{
    std::string threshold_var = this->threshold_variable;

    if (threshold_var.empty() &&
        request.has("dagr_connected_components::threshold_variable"))
            request.get("dagr_connected_components::threshold_variable",
                threshold_var);
...
// --------------------------------------------------------------------------
dagr_metadata dagr_connected_components::get_output_metadata(
    unsigned int port,
    const std::vector<dagr_metadata> &input_md)
{
#ifdef DAGR_DEBUG
    cerr << dagr_parallel_id()
        << "dagr_connected_components::get_output_metadata" << endl;
#endif
    (void) port;
...
// --------------------------------------------------------------------------
std::vector<dagr_metadata> dagr_connected_components::get_upstream_request(
    unsigned int port,
    const std::vector<dagr_metadata> &input_md,
    const dagr_metadata &request)
{
#ifdef DAGR_DEBUG
    cerr << dagr_parallel_id()
        << "dagr_connected_components::get_upstream_request" << endl;
#endif
    (void) port;
    (void) input_md;
...
// --------------------------------------------------------------------------
const_p_dagr_dataset dagr_connected_components::execute(
    unsigned int port,
    ...
    const dagr_metadata &request)
{
#ifdef DAGR_DEBUG
    cerr << dagr_parallel_id()
        << "dagr_connected_components::execute" << endl;
#endif
    (void)port;
    ...
    if (low == std::numeric_limits<double>::lowest()
        && request.has("dagr_connected_components::low_threshold_value"))
        request.get("dagr_connected_components::low_threshold_value", low);

    double high = this->high_threshold_value;
    if (high == std::numeric_limits<double>::max()
        && request.has("dagr_connected_components::high_threshold_value"))
        request.get("dagr_connected_components::high_threshold_value", high);
    ...
    p_dagr_short_array labels = dagr_short_array::New(n_elem);
