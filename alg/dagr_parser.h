#ifndef dagr_parser_h
#define dagr_parser_h
...
#define DAGR_PARSER_ERROR(_descr, _expr, _pos)                          \
    DAGR_MESSAGE("ERROR:", ANSI_RED,                                    \
        << "" _descr                                                    \
        << "at position " << (_pos) << " in  \""                        \
...
#define DAGR_SYNTAX_ERROR(_expr, _pos)                                  \
    DAGR_PARSER_ERROR("Syntax error. ", _expr, _pos)

#define DAGR_NAME_RESOLUTION_ERROR(_name, _expr, _pos)                  \
    DAGR_PARSER_ERROR("Name resolution error \""                        \
        << _name << "\". ", _expr, _pos)

#define DAGR_INVALID_OPERATION_ERROR(_op, _expr, _pos)                  \
    DAGR_PARSER_ERROR("Invalid operation \""                            \
        << _op << "\". ", _expr, _pos)

#define DAGR_OPERATION_FAILED_ERROR(_op, _expr, _pos)                   \
    DAGR_PARSER_ERROR("Operation \"" << _op                             \
        << "\" failed. " , _expr, _pos)

#define DAGR_NUM_OPERANDS_ERROR(_op, _nreq, _ngive, _expr, _pos)        \
    DAGR_PARSER_ERROR("Operation \"" << _op                             \
         << "\" requires " << _nreq << " operands, given "              \
         << _ngive << ". ", _expr, _pos)

namespace dagr_parser
{
/**
...
*/
template<typename tokenizer_t=dagr_parser::tokenizer>
char *infix_to_postfix(const char *iexpr, std::set<std::string> *variables)
{
    ...
    while (*expr)
    {
        ...
        {
            DAGR_SYNTAX_ERROR(iexpr, expr-iexpr)
            return nullptr;
        }
    }
    ...
    {
        DAGR_SYNTAX_ERROR(iexpr, group_position.back())
        return nullptr;
    }
...
typename operator_resolver_t, typename tokenizer_t=dagr_parser::tokenizer>
    ...
    const char *iexpr, operand_resolver_t &operands)
{
    ...
    while (*expr)
    {
        ...
        else if ((token_len = tokenizer_t::get_constant_name(expr, token)))
        {
            ...
            {
                DAGR_NAME_RESOLUTION_ERROR(token, iexpr, expr-iexpr)
                return -1;
            }
        ...
        else if ((token_len = tokenizer_t::get_ternary_operator_name(expr, token)))
        {
            ...
            if (n_operands < 3)
            {
                DAGR_NUM_OPERANDS_ERROR(token, 3, n_operands, iexpr, expr-iexpr)
                return -1;
            }
            ...
            if ((err_code = operator_resolver_t::invoke(token, result, arg1, arg2, arg3)))
            {
                if (err_code == -1)
                {
                    DAGR_INVALID_OPERATION_ERROR(token, iexpr, expr-iexpr)
                }
                else if (err_code == -2)
                {
                    DAGR_OPERATION_FAILED_ERROR(token, iexpr, expr-iexpr)
                }
                return -1;
            }
        ...
        else if ((token_len = tokenizer_t::get_binary_operator_name(expr, token)))
        {
            ...
            if (n_operands < 2)
            {
                DAGR_NUM_OPERANDS_ERROR(token, 2, n_operands, iexpr, expr-iexpr)
                return -1;
            }
        ...
        else if ((token_len = tokenizer_t::get_unary_operator_name(expr, token)))
        {
            ...
            if (n_operands < 1)
            {
                DAGR_NUM_OPERANDS_ERROR(token, 1, n_operands, iexpr, expr-iexpr)
                return -1;
            }
