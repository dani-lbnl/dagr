    ...
    DT &val)
{
    ...
    if ((ihi && dagr_coordinate_util::index_of(p_x, 0, ihi, cx, true, i))
        || (jhi && dagr_coordinate_util::index_of(p_y, 0, jhi, cy, true, j))
        || (khi && dagr_coordinate_util::index_of(p_z, 0, khi, cz, true, k)))
    {
...
// --------------------------------------------------------------------------
dagr_cartesian_mesh_regrid::dagr_cartesian_mesh_regrid()
...
// --------------------------------------------------------------------------
void dagr_cartesian_mesh_regrid::get_properties_description(
    const string &prefix, options_description &global_opts)
{
    options_description opts("Options for "
        + (prefix.empty()?"dagr_cartesian_mesh_regrid":prefix));

    opts.add_options()
        DAGR_POPTS_GET(vector<string>, prefix, source_arrays,
            "list of arrays to move from source to target mesh")
        DAGR_POPTS_GET(int, prefix, interpolation_mode,
...
// --------------------------------------------------------------------------
void dagr_cartesian_mesh_regrid::set_properties(
    const string &prefix, variables_map &opts)
{
    DAGR_POPTS_SET(opts, vector<string>, prefix, source_arrays)
    DAGR_POPTS_SET(opts, int, prefix, interpolation_mode)
}

#endif

// --------------------------------------------------------------------------
void dagr_cartesian_mesh_regrid::clear_source_arrays()
{
...
// --------------------------------------------------------------------------
void dagr_cartesian_mesh_regrid::add_source_array(const std::string &array)
{
...
// --------------------------------------------------------------------------
dagr_metadata dagr_cartesian_mesh_regrid::get_output_metadata(
    unsigned int port,
...
// --------------------------------------------------------------------------
std::vector<dagr_metadata> dagr_cartesian_mesh_regrid::get_upstream_request(
    unsigned int port, const std::vector<dagr_metadata> &input_md,
    const dagr_metadata &request)
{
    (void)port;

    vector<dagr_metadata> up_reqs(2);

    // compose the target request
    dagr_metadata target_req(request);
    ...
    dagr_metadata target_coords;
    p_dagr_variant_array target_x;
    p_dagr_variant_array target_y;
    p_dagr_variant_array target_z;
    ...
    {
        DAGR_ERROR("failed to locate target mesh coordinates")
        return up_reqs;
    }
    ...
    if (request.get("bounds", request_bounds, 6))
    {
        ...
        {
            DAGR_ERROR("neither \"bounds\" nor \"extent\" has been requested")
            return up_reqs;
        }
    }
    else
    {
        if (dagr_coordinate_util::bounds_to_extent(request_bounds,
            target_x, target_y, target_z, target_extent))
        {
            DAGR_ERROR("invalid bounds requested.")
            return up_reqs;
        }
    }
    ...
    dagr_metadata source_req(target_req);
...
// --------------------------------------------------------------------------
const_p_dagr_dataset dagr_cartesian_mesh_regrid::execute(
    unsigned int port,
    ...
    const dagr_metadata &request)
{
#ifdef DAGR_DEBUG
    cerr << dagr_parallel_id()
        << "dagr_cartesian_mesh_regrid::execute" << endl;
#endif
    (void)port;
    ...
    const_p_dagr_cartesian_mesh source
        = std::dynamic_pointer_cast<const dagr_cartesian_mesh>(input_data[1]);

    if (!in_target || !source)
    {
        DAGR_ERROR("invalid input. target invalid "
    ...
    // move the arrays
    const_p_dagr_variant_array target_xc = target->get_x_coordinates();
    const_p_dagr_variant_array target_yc = target->get_y_coordinates();
    const_p_dagr_variant_array target_zc = target->get_z_coordinates();
    p_dagr_array_collection target_ac = target->get_point_arrays();
    ...
    const_p_dagr_variant_array source_xc = source->get_x_coordinates();
    const_p_dagr_variant_array source_yc = source->get_y_coordinates();
    const_p_dagr_variant_array source_zc = source->get_z_coordinates();
    const_p_dagr_array_collection source_ac = source->get_point_arrays();
    ...
    NESTED_TEMPLATE_DISPATCH_FP(
        ...
        NESTED_TEMPLATE_DISPATCH_FP(
            ...
            {
                const_p_dagr_variant_array source_a = source_ac->get(source_arrays[i]);
                p_dagr_variant_array target_a = source_a->new_instance();
                        ...
                        for (unsigned long j = 0; j < target_ny; ++j)
                        {
                            for (unsigned long i = 0; i < target_nx; ++i, ++q)
                            {
                                if (interpolate_nearest<NT2,NT3>(
                                    ...
                                    p_target_a[q]))
                                {
                                    DAGR_ERROR("failed to interpolate " << i << ", " << j << ", " << k)
                                    return nullptr;
                                }
                            }
                        }
