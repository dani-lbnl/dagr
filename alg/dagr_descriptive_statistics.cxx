...
// --------------------------------------------------------------------------
dagr_descriptive_statistics::dagr_descriptive_statistics()
{
...
#if defined(DAGR_HAS_BOOST)
// --------------------------------------------------------------------------
void dagr_descriptive_statistics::get_properties_description(
    const string &prefix, options_description &global_opts)
{
    options_description opts("Options for "
        + (prefix.empty()?"dagr_descriptive_statistics":prefix));
...
// --------------------------------------------------------------------------
void dagr_descriptive_statistics::set_properties(
...
// --------------------------------------------------------------------------
void dagr_descriptive_statistics::get_dependent_variables(
    const dagr_metadata &request, std::vector<std::string> &dep_vars)
{
    ...
    if (dep_vars.empty())
    {
        std::string key = "dagr_descriptive_statistics::dependent_variables";
        if (request.has(key))
...
// --------------------------------------------------------------------------
const_p_dagr_dataset dagr_descriptive_statistics::execute(
    unsigned int port,
    const std::vector<const_p_dagr_dataset> &input_data,
    const dagr_metadata &request)
{
#ifdef DAGR_DEBUG
    cerr << dagr_parallel_id() << "dagr_descriptive_statistics::execute" << endl;
#endif
    (void)port;
    ...
    if (!in_mesh)
    {
        DAGR_ERROR("dataset is not a dagr_cartesian_mesh")
        return nullptr;
    }
    ...
    for (size_t i = 0; i < n_dep_vars; ++i)
    {
        ...
        // get the array
        const_p_dagr_variant_array dep_var
            = in_mesh->get_point_arrays()->get(dep_var_name);
        if (!dep_var)
        {
            DAGR_ERROR("dependent variable " << i << " \""
