#ifndef dagr_binary_stream_h
#define dagr_binary_stream_h
...
public:
    // construct
    dagr_binary_stream();
    ~dagr_binary_stream() noexcept;

    // copy
    dagr_binary_stream(const dagr_binary_stream &s);
    const dagr_binary_stream &operator=(const dagr_binary_stream &other);

    // move
    dagr_binary_stream(dagr_binary_stream &&s) noexcept;
    const dagr_binary_stream &operator=(dagr_binary_stream &&other) noexcept;
    ...
    void swap(dagr_binary_stream &other) noexcept;
...
//-----------------------------------------------------------------------------
template <typename T>
void dagr_binary_stream::pack(T *val)
{
    (void)val;
    DAGR_ERROR("Error: Packing a pointer.");
}

//-----------------------------------------------------------------------------
template <typename T>
void dagr_binary_stream::pack(const T &val)
{
    this->grow(sizeof(T));
    *((T *)m_write_p) = val;
    m_write_p += sizeof(T);
...
//-----------------------------------------------------------------------------
template <typename T>
void dagr_binary_stream::unpack(T &val)
{
    val = *((T *)m_read_p);
    m_read_p += sizeof(T);
}
...
//-----------------------------------------------------------------------------
template <typename T>
void dagr_binary_stream::pack(const T *val, unsigned long n)
{
...
//-----------------------------------------------------------------------------
template <typename T>
void dagr_binary_stream::unpack(T *val, unsigned long n)
{
    unsigned long nn = n*sizeof(T);
...
//-----------------------------------------------------------------------------
inline
void dagr_binary_stream::pack(const std::string &str)
{
...
//-----------------------------------------------------------------------------
inline
void dagr_binary_stream::unpack(std::string &str)
{
...
//-----------------------------------------------------------------------------
inline
void dagr_binary_stream::pack(const std::vector<std::string> &v)
{
...
//-----------------------------------------------------------------------------
inline
void dagr_binary_stream::unpack(std::vector<std::string> &v)
{
    unsigned long vlen;
    this->unpack(vlen);
...
//-----------------------------------------------------------------------------
template<typename T>
void dagr_binary_stream::pack(const std::vector<T> &v)
{
...
//-----------------------------------------------------------------------------
template<typename T>
void dagr_binary_stream::unpack(std::vector<T> &v)
{
    unsigned long vlen;
    this->unpack(vlen);
