...
int send(MPI_Comm comm, int dest, dagr_binary_stream &s)
{
    unsigned long long n = s.size();
    if (MPI_Send(&n, 1, MPI_UNSIGNED_LONG_LONG, dest, 3210, comm))
    {
        DAGR_ERROR("failed to send send message size")
        return -1;
    }
    ...
    if (n)
    {
        ...
        {
            DAGR_ERROR("failed to send message")
            return -2;
        }
    }
...
int recv(MPI_Comm comm, int src, dagr_binary_stream &s)
{
    size_t n = 0;
    MPI_Status stat;
    if (MPI_Recv(&n, 1, MPI_UNSIGNED_LONG_LONG, src, 3210, comm, &stat))
    {
        DAGR_ERROR("failed to receive message size")
        return -2;
    }
    ...
    if (n)
    {
        ...
        {
            DAGR_ERROR("failed to receive message")
            return -2;
        }
    }
    ...
    bool verbose)
{
    ...
    if (verbose)
    {
        ...
        if (rank == 0)
        {
            ...
            DAGR_STATUS("map index decomposition:"
                << std::endl << oss.str())
        }
    }
}
...
// --------------------------------------------------------------------------
dagr_temporal_reduction::dagr_temporal_reduction()
    : first_step(0), last_step(-1)
{}

#if defined(DAGR_HAS_BOOST)
// --------------------------------------------------------------------------
void dagr_temporal_reduction::get_properties_description(const std::string &prefix,
    options_description &global_opts)
{
    this->dagr_threaded_algorithm::get_properties_description(prefix, global_opts);

    options_description opts("Options for "
        + (prefix.empty()?"dagr_temporal_reduction":prefix));

    opts.add_options()
        DAGR_POPTS_GET(long, prefix, first_step, "first time step to process (0)")
        DAGR_POPTS_GET(long, prefix, last_step, "last time step to process. "
...
// --------------------------------------------------------------------------
void dagr_temporal_reduction::set_properties(const std::string &prefix,
    variables_map &opts)
{
    this->dagr_threaded_algorithm::set_properties(prefix, opts);

    DAGR_POPTS_SET(opts, long, prefix, first_step)
    DAGR_POPTS_SET(opts, long, prefix, last_step)
}
#endif

// --------------------------------------------------------------------------
std::vector<dagr_metadata> dagr_temporal_reduction::get_upstream_request(
    unsigned int port, const std::vector<dagr_metadata> &input_md,
    const dagr_metadata &request)
{
    std::vector<dagr_metadata> up_req;
    ...
    std::vector<dagr_metadata> base_req
...
// --------------------------------------------------------------------------
dagr_metadata dagr_temporal_reduction::get_output_metadata(
    unsigned int port,
    const std::vector<dagr_metadata> &input_md)
{
    dagr_metadata output_md
...
// --------------------------------------------------------------------------
const_p_dagr_dataset dagr_temporal_reduction::reduce_local(
    std::vector<const_p_dagr_dataset> input_data) // pass by value is intentional
{
...
// --------------------------------------------------------------------------
const_p_dagr_dataset dagr_temporal_reduction::reduce_remote(
    const_p_dagr_dataset local_data) // pass by value is intentional
{
#if defined(DAGR_HAS_MPI)
    int is_init = 0;
    MPI_Initialized(&is_init);
    if (is_init)
    {
        ...
        if (left_id <= n_ranks)
        {
            ...
            {
                DAGR_ERROR("failed to recv from left")
                return p_dagr_dataset();
            }

            p_dagr_dataset left_data;
            if (local_data && bstr)
            {
        ...
        if (right_id <= n_ranks)
        {
            ...
            {
                DAGR_ERROR("failed to recv from right")
                return p_dagr_dataset();
            }

            p_dagr_dataset right_data;
            if (local_data && bstr)
            {
...
// --------------------------------------------------------------------------
const_p_dagr_dataset dagr_temporal_reduction::execute(
    unsigned int port,
