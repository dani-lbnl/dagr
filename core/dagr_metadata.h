#ifndef dagr_metadata_h
#define dagr_metadata_h
...
{
public:
    dagr_metadata() noexcept;
    ~dagr_metadata() noexcept;

    dagr_metadata(const dagr_metadata &other);
    dagr_metadata &operator=(const dagr_metadata &other);

    dagr_metadata(dagr_metadata &&other) noexcept;
    dagr_metadata &operator=(dagr_metadata &&other) noexcept;
    ...
    int get(const std::string &name, p_dagr_variant_array val) const;

    // get the variant array, or nullptr if the
    // property doesn't exist
    p_dagr_variant_array get(const std::string &name);
    const_p_dagr_variant_array get(const std::string &name) const;
    ...
    void to_stream(dagr_binary_stream &s) const;
    void from_stream(dagr_binary_stream &s);
...
private:
    ...
    using prop_map_t = std::map<std::string, p_dagr_variant_array>;
    prop_map_t props;

    friend bool operator<(const dagr_metadata &, const dagr_metadata &) noexcept;
    friend bool operator==(const dagr_metadata &, const dagr_metadata &) noexcept;
    friend dagr_metadata operator&(const dagr_metadata &, const dagr_metadata &);
};
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_metadata::declare(const std::string &name)
{
    p_dagr_variant_array prop_val
        = dagr_variant_array_impl<T>::New();
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_metadata::declare(const std::string &name, unsigned int n)
{
    p_dagr_variant_array prop_val
        = dagr_variant_array_impl<T>::New(n);
...
// --------------------------------------------------------------------------
template<typename T>
int dagr_metadata::append(const std::string &name, const T &val)
{
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_metadata::insert(const std::string &name, const T &val)
{
    p_dagr_variant_array prop_val
        = dagr_variant_array_impl<T>::New(&val, 1);
...
{
    p_dagr_variant_array prop_val
        = dagr_variant_array_impl<T>::New(vals, n_vals);
    ...
    const std::string &name, const std::set<T> &vals)
{
    ...
    p_dagr_variant_array prop_val
        = dagr_variant_array_impl<T>::New(tmp.data(), n);
    ...
    const std::vector<T> &vals)
{
    ...
    p_dagr_variant_array prop_val
        = dagr_variant_array_impl<T>::New(vals.data(), n);
    ...
    const std::vector<std::vector<T>> &vals)
{
    ...
    p_dagr_variant_array prop_vals
        = dagr_variant_array_impl<p_dagr_variant_array>::New();

    for (size_t i = 0; i < n; ++i)
    {
        p_dagr_variant_array prop_val
            = dagr_variant_array_impl<T>::New((vals.at(i).data()), vals.at(i).size());
...
// --------------------------------------------------------------------------
template<typename T>
int dagr_metadata::set(const std::string &name, unsigned int i, const T &val)
{
    prop_map_t::iterator it = this->props.find(name);
    if (it == this->props.end())
    {
...
// --------------------------------------------------------------------------
template<typename T>
int dagr_metadata::get(const std::string &name, unsigned int i, T &val) const
{
...
using dagr_metadata_array = dagr_variant_array_impl<dagr_metadata>;
#endif
