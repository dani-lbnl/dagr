#ifndef dagr_variant_array_h
#define dagr_variant_array_h
...
type agnostic container for array based data.
*/
class dagr_variant_array : public std::enable_shared_from_this<dagr_variant_array>
{
public:
    // construct
    dagr_variant_array() noexcept = default;
    virtual ~dagr_variant_array() noexcept = default;

    // copy/move construct. can't copy/move construct from a base
    // pointer. these require a downcast.
    dagr_variant_array(const dagr_variant_array &other) = delete;
    dagr_variant_array(dagr_variant_array &&other) = delete;

    // copy assign
    dagr_variant_array &operator=(const dagr_variant_array &other)
    { this->copy(other); return *this; }

    // move assign
    dagr_variant_array &operator=(dagr_variant_array &&other)
    { this->swap(other); return *this; }

    // virtual constructor. return a new'ly allocated
    // empty object of the same type.
    virtual p_dagr_variant_array new_instance() const = 0;
    virtual p_dagr_variant_array new_instance(size_t n) const = 0;

    // virtual copy construct. return a new'ly allocated object,
    // initialized copy from this. caller must delete.
    virtual p_dagr_variant_array new_copy() const = 0;
    virtual p_dagr_variant_array new_copy(size_t start, size_t end) const = 0;

    // return true if values are equal
    bool operator==(const dagr_variant_array &other) const
    ...
    void copy(const dagr_variant_array &other);
    void copy(const const_p_dagr_variant_array &other)
    { this->copy(*other.get()); }

    void append(const dagr_variant_array &other);
    void append(const const_p_dagr_variant_array &other)
    { this->append(*other.get()); }

    // swap the contents of this and the other object.
    // an excpetion is thrown when no conversion
    // between the two types exists.
    virtual void swap(dagr_variant_array &other) = 0;
    void swap(const p_dagr_variant_array &other)
    { this->swap(*other.get()); }

    // compare the two objects for equality
    virtual bool equal(const dagr_variant_array &other) const = 0;
    bool equal(const const_p_dagr_variant_array &other) const
    { return this->equal(*other.get()); }

    // serrialize to/from stream
    virtual void to_stream(dagr_binary_stream &s) const = 0;
    virtual void from_stream(dagr_binary_stream &s) = 0;
...
// overrides in dagr_binary_stream
template<typename T>
struct pack_array
    : std::integral_constant<bool,
    std::is_arithmetic<T>::value ||
...
struct pack_object_ptr
    ...
    std::is_same<T, p_dagr_variant_array>::value) &&
    !pack_array<T>::value>
{};
...
// for simple arrays
template<typename T>
class dagr_variant_array_impl : public dagr_variant_array
{
public:
    // construct
    DAGR_VARIANT_ARRAY_STATIC_NEW(dagr_variant_array_impl, T)

    // destruct
    virtual ~dagr_variant_array_impl() noexcept;

    // virtual constructor
    virtual p_dagr_variant_array new_copy() const override;
    virtual p_dagr_variant_array new_copy(size_t start, size_t end) const override;
    virtual p_dagr_variant_array new_instance() const override;
    virtual p_dagr_variant_array new_instance(size_t n) const override;

    // copy
    const dagr_variant_array_impl<T> &
    operator=(const dagr_variant_array_impl<T> &other);

    template<typename U>
    const dagr_variant_array_impl<T> &
    operator=(const dagr_variant_array_impl<U> &other);

    // move
    dagr_variant_array_impl(dagr_variant_array_impl<T> &&other);

    const dagr_variant_array_impl<T> &
    operator=(dagr_variant_array_impl<T> &&other);
    ...
    // virtual swap
    virtual void swap(dagr_variant_array &other) override;

    // virtual equavalince test
    virtual bool equal(const dagr_variant_array &other) const override;

    // serialize to/from stream
    virtual void to_stream(dagr_binary_stream &s) const override
    { this->to_binary<T>(s); }

    virtual void from_stream(dagr_binary_stream &s) override
...
protected:
    // construct
    dagr_variant_array_impl() noexcept {}

    // construct with preallocated size
    dagr_variant_array_impl(unsigned long n)
        : m_data(n) {}

    // construct with preallocated size and initialized
    // to a specific value
    dagr_variant_array_impl(unsigned long n, const T &v)
        : m_data(n, v) {}

    // construct from a c-array of length n
    dagr_variant_array_impl(const T *vals, unsigned long n)
        : dagr_variant_array(), m_data(vals, vals+n) {}

    // copy construct from an instance of different type
    template<typename U>
    dagr_variant_array_impl(const dagr_variant_array_impl<U> &other)
        : dagr_variant_array(), m_data(other.m_data) {}

    // copy construct from an instance of same type
    dagr_variant_array_impl(const dagr_variant_array_impl<T> &other)
        : dagr_variant_array(), m_data(other.m_data) {}
...
private:
    ...
    void to_binary(dagr_binary_stream &s,
        typename std::enable_if<pack_array<U>::value, U>::type* = 0)
        const;

    template <typename U = T>
    void from_binary(dagr_binary_stream &s,
        typename std::enable_if<pack_array<U>::value, U>::type* = 0);
...
private:
    ...
    friend class dagr_variant_array;
    template<typename U> friend class dagr_variant_array_impl;
};
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array::get_dispatch(std::vector<T> &vals,
    typename std::enable_if<pod_dispatch<T>::value, T>::type*) const
{
    TEMPLATE_DISPATCH(const dagr_variant_array_impl, this,
...
{
    TEMPLATE_DISPATCH_CASE(const dagr_variant_array_impl, T, this,
        TT *this_t = static_cast<TT*>(this);
        this_t->get(vals);
        return;
        )
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array::get_dispatch(unsigned long i, T &val,
    typename std::enable_if<pod_dispatch<T>::value, T>::type*) const
{
    TEMPLATE_DISPATCH(const dagr_variant_array_impl, this,
        TT *this_t = static_cast<TT*>(this);
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array::get_dispatch(size_t start, size_t end, T *vals,
    typename std::enable_if<pod_dispatch<T>::value, T>::type*) const
{
    TEMPLATE_DISPATCH(const dagr_variant_array_impl, this,
        TT *this_t = static_cast<TT*>(this);
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array::set_dispatch(const std::vector<T> &vals,
...
{
    TEMPLATE_DISPATCH_CASE(dagr_variant_array_impl, T, this,
        TT *this_t = static_cast<TT*>(this);
        this_t->set(vals);
        return;
        )
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array::set_dispatch(unsigned long i, const T &val,
    typename std::enable_if<pod_dispatch<T>::value, T>::type*)
{
    TEMPLATE_DISPATCH(dagr_variant_array_impl, this,
        TT *this_t = static_cast<TT*>(this);
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array::set_dispatch(size_t start, size_t end, const T *vals,
    typename std::enable_if<pod_dispatch<T>::value, T>::type*)
{
    TEMPLATE_DISPATCH(dagr_variant_array_impl, this,
        TT *this_t = static_cast<TT*>(this);
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array::append_dispatch(const std::vector<T> &vals,
    typename std::enable_if<pod_dispatch<T>::value, T>::type*)
{
    TEMPLATE_DISPATCH(dagr_variant_array_impl, this,
        TT *this_t = static_cast<TT*>(this);
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array::append_dispatch(const T &val,
    typename std::enable_if<pod_dispatch<T>::value, T>::type*)
{
    TEMPLATE_DISPATCH(dagr_variant_array_impl, this,
        TT *this_t = static_cast<TT*>(this);
...
// --------------------------------------------------------------------------
template<typename T>
dagr_variant_array_impl<T>::~dagr_variant_array_impl() noexcept
{
    this->clear();
}
...
// --------------------------------------------------------------------------
template<typename T>
p_dagr_variant_array dagr_variant_array_impl<T>::new_copy() const
{
    return p_dagr_variant_array(new dagr_variant_array_impl<T>(*this));
}

// --------------------------------------------------------------------------
template<typename T>
p_dagr_variant_array dagr_variant_array_impl<T>::new_copy(
    size_t start, size_t end) const
{
    p_dagr_variant_array_impl<T> c = dagr_variant_array_impl<T>::New(end-start+1);
...
// --------------------------------------------------------------------------
template<typename T>
p_dagr_variant_array dagr_variant_array_impl<T>::new_instance() const
{
    return p_dagr_variant_array(new dagr_variant_array_impl<T>());
}

// --------------------------------------------------------------------------
template<typename T>
p_dagr_variant_array dagr_variant_array_impl<T>::new_instance(size_t n) const
{
    return p_dagr_variant_array(new dagr_variant_array_impl<T>(n));
}

// --------------------------------------------------------------------------
template<typename T>
const dagr_variant_array_impl<T> &
dagr_variant_array_impl<T>::operator=(const dagr_variant_array_impl<T> &other)
{
...
template<typename U>
const dagr_variant_array_impl<T> &
dagr_variant_array_impl<T>::operator=(const dagr_variant_array_impl<U> &other)
{
    m_data.assign(other.m_data.begin(), other.m_data.end());
    return *this;
}
...
// --------------------------------------------------------------------------
template<typename T>
dagr_variant_array_impl<T>::dagr_variant_array_impl(
    dagr_variant_array_impl<T> &&other)
    : m_data(std::move(other.m_data))
{}

// --------------------------------------------------------------------------
template<typename T>
const dagr_variant_array_impl<T> &dagr_variant_array_impl<T>::operator=(
    dagr_variant_array_impl<T> &&other)
{
    m_data = std::move(other.m_data);
    return *this;
...
template<typename T>
template<typename U>
void dagr_variant_array_impl<T>::get(unsigned long i, U &val) const
{
    val = m_data[i];
}
...
template<typename T>
template<typename U>
void dagr_variant_array_impl<T>::get(size_t start, size_t end, U *vals) const
{
...
template<typename T>
template<typename U>
void dagr_variant_array_impl<T>::get(std::vector<U> &val) const
{
...
template<typename T>
template<typename U>
void dagr_variant_array_impl<T>::set(unsigned long i, const U &val)
{
    m_data[i] = val;
}
...
template<typename T>
template<typename U>
void dagr_variant_array_impl<T>::set(size_t start, size_t end, const U *vals)
{
    for (size_t i = start, ii = 0; i <= end; ++i, ++ii)
        m_data[i] = vals[ii];
}
...
template<typename T>
template<typename U>
void dagr_variant_array_impl<T>::set(const std::vector<U> &val)
{
    size_t n = val.size();
    m_data.resize(n);
...
template<typename T>
template<typename U>
void dagr_variant_array_impl<T>::append(const std::vector<U> &val)
{
...
template<typename T>
template<typename U>
void dagr_variant_array_impl<T>::append(const U &val)
{
...
// --------------------------------------------------------------------------
template<typename T>
unsigned long dagr_variant_array_impl<T>::size() const noexcept
{ return m_data.size(); }

// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array_impl<T>::resize(unsigned long n)
{
    m_data.resize(n);
}
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array_impl<T>::resize(unsigned long n, const T &val)
{
    m_data.resize(n, val);
}
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array_impl<T>::reserve(unsigned long n)
{
    m_data.reserve(n);
}
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array_impl<T>::clear() noexcept
{
    m_data.clear();
}
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array_impl<T>::copy(const dagr_variant_array &other)
{
    TEMPLATE_DISPATCH(const dagr_variant_array_impl, &other,
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array_impl<T>::append(const dagr_variant_array &other)
{
    TEMPLATE_DISPATCH(const dagr_variant_array_impl, &other,
        TT *other_t = static_cast<TT*>(&other);
        std::copy(
...
// --------------------------------------------------------------------------
template<typename T>
void dagr_variant_array_impl<T>::swap(dagr_variant_array &other)
{
    using TT = dagr_variant_array_impl<T>;
...
// --------------------------------------------------------------------------
template<typename T>
bool dagr_variant_array_impl<T>::equal(const dagr_variant_array &other) const
{
    using TT = dagr_variant_array_impl<T>;
...
template<typename T>
    template <typename U>
void dagr_variant_array_impl<T>::to_binary(
    dagr_binary_stream &s,
...
template<typename T>
    template <typename U>
void dagr_variant_array_impl<T>::from_binary(
    dagr_binary_stream &s,
...
template<typename T>
    template <typename U>
void dagr_variant_array_impl<T>::to_ascii(
    std::ostream &s,
    typename std::enable_if<pack_array<U>::value, U>::type*) const
{
...
template<typename T>
    template <typename U>
void dagr_variant_array_impl<T>::from_ascii(
    std::ostream &,
    typename std::enable_if<pack_array<U>::value, U>::type*)
{
    // TODO
...
template <typename T>
struct dagr_variant_array_code
{}; // intentionally empty

template <unsigned int I>
struct dagr_variant_array_new
{}; // intentionally empty

#define DAGR_VARIANT_ARRAY_TT_SPEC(T, v)            \
template <>                                         \
struct dagr_variant_array_code<T>                   \
{                                                   \
    static unsigned int get() noexcept              \
    { return v; }                                   \
};                                                  \
template <>                                         \
struct dagr_variant_array_new<v>                    \
{                                                   \
    static p_dagr_variant_array_impl<T> New()       \
    { return dagr_variant_array_impl<T>::New(); }   \
};

#define DAGR_VARIANT_ARRAY_FACTORY_NEW(_code)               \
        case _code:                                         \
            return dagr_variant_array_new<_code>::New();

#include "dagr_metadata.h"
class dagr_metadata;

DAGR_VARIANT_ARRAY_TT_SPEC(char, 1)
DAGR_VARIANT_ARRAY_TT_SPEC(unsigned char, 2)
DAGR_VARIANT_ARRAY_TT_SPEC(int, 3)
DAGR_VARIANT_ARRAY_TT_SPEC(unsigned int, 4)
DAGR_VARIANT_ARRAY_TT_SPEC(short int, 5)
DAGR_VARIANT_ARRAY_TT_SPEC(short unsigned int, 6)
DAGR_VARIANT_ARRAY_TT_SPEC(long, 7)
DAGR_VARIANT_ARRAY_TT_SPEC(unsigned long, 8)
DAGR_VARIANT_ARRAY_TT_SPEC(long long, 9)
DAGR_VARIANT_ARRAY_TT_SPEC(unsigned long long, 10)
DAGR_VARIANT_ARRAY_TT_SPEC(float, 11)
DAGR_VARIANT_ARRAY_TT_SPEC(double, 12)
DAGR_VARIANT_ARRAY_TT_SPEC(std::string, 13)
DAGR_VARIANT_ARRAY_TT_SPEC(dagr_metadata, 14)
DAGR_VARIANT_ARRAY_TT_SPEC(p_dagr_variant_array, 15)

struct dagr_variant_array_factory
{
    static p_dagr_variant_array New(unsigned int type_code)
    {
        switch (type_code)
        {
        DAGR_VARIANT_ARRAY_FACTORY_NEW(1)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(2)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(3)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(4)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(5)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(6)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(7)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(8)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(9)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(10)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(11)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(12)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(13)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(14)
        DAGR_VARIANT_ARRAY_FACTORY_NEW(15)
        default:
            DAGR_ERROR(
...
// --------------------------------------------------------------------------
template<typename T>
unsigned int dagr_variant_array_impl<T>::type_code() const noexcept
{
    return dagr_variant_array_code<T>::get();
}
