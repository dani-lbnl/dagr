#ifndef dagr_algorithm_h
#define dagr_algorithm_h
...
// interface to dagr pipeline architecture. all sources/readers
// filters, sinks/writers will implement this interface
class dagr_algorithm : public std::enable_shared_from_this<dagr_algorithm>
{
public:
    // construct/destruct
    static p_dagr_algorithm New();
    virtual ~dagr_algorithm() noexcept;
    DAGR_ALGORITHM_DELETE_COPY_ASSIGN(dagr_algorithm)
    ...
    virtual
    dagr_algorithm_output_port get_output_port(unsigned int port = 0);

    // set an input to this algorithm
    void set_input_connection(const dagr_algorithm_output_port &port)
    ...
    const_p_dagr_dataset get_output_data(unsigned int port = 0);
    ...
    virtual dagr_metadata update_metadata(unsigned int port = 0);

    // set the executive
    void set_executive(p_dagr_algorithm_executive exe);
...
// dagr_algorithm's such as reader, filters, and
// writers.
    ...
    // request.
    virtual
    dagr_metadata get_cache_key(unsigned int port,
...
// of algorithms.
    ...
    virtual
    const_p_dagr_dataset request_data(
        dagr_algorithm_output_port &port,
        const dagr_metadata &request);
    ...
    // port upstream.
    virtual
    int validate_cache(dagr_algorithm_output_port &current);

    // driver function that clears the modified flag on the
    // named port and all of it's upstream connections.
    virtual
    void clear_modified(dagr_algorithm_output_port current);
...
// api exposing internals for use in driver methods
    ...
    int cache_output_data(unsigned int port,
        const dagr_metadata &request, const_p_dagr_dataset &data);
    ...
    dagr_algorithm_output_port &get_input_connection(unsigned int i);
...
private:
    dagr_algorithm_internals *internals;

    friend class dagr_threaded_algorithm;
    friend class dagr_data_request;
};
