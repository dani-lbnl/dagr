...
// --------------------------------------------------------------------------
dagr_metadata array_add::get_output_metadata(
    unsigned int port,
    const std::vector<dagr_metadata> &input_md)
{
    ...
    {
        DAGR_ERROR("failed to get the active arrays")
        return output_md;
    }
...
// --------------------------------------------------------------------------
std::vector<dagr_metadata> array_add::get_upstream_request(
    unsigned int port,
    const std::vector<dagr_metadata> &input_md,
    const dagr_metadata &request)
{
    ...
    dagr_metadata up_req_1(request);
    up_req_1.insert("array_name", active_array_1);
    up_reqs[0] = up_req_1;

    dagr_metadata up_req_2(request);
...
// --------------------------------------------------------------------------
const_p_dagr_dataset array_add::execute(
    unsigned int port,
    ...
    const dagr_metadata &request)
{
    ...
    if (!a_in_1 || !a_in_2)
    {
        DAGR_ERROR("required inputs are not present")
        return a_out;
    }
    ...
    {
        DAGR_ERROR("failed to get the active array")
        return a_out;
    }
