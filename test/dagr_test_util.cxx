...
// **************************************************************************
const_p_dagr_table create_test_table(long step, int tid)
{
    p_dagr_table t = dagr_table::New();
    ...
    switch (tid)
    {
        case dagr_test_util::base_table:
            return t;
            break;

        case dagr_test_util::break_string_col:
            std::dynamic_pointer_cast<dagr_variant_array_impl<std::string>>(
                t->get_column("name"))->set(2, "Dave");
            return t;
            break;

        case dagr_test_util::break_int_col:
            std::dynamic_pointer_cast<dagr_variant_array_impl<std::string>>(
                t->get_column("age"))->set(2, 6);
            return t;
            break;

        case dagr_test_util::break_float_col:
            std::dynamic_pointer_cast<dagr_variant_array_impl<std::string>>(
                t->get_column("skill"))->set(2, 2.71828183);
            return t;
    }

    DAGR_ERROR("bad table id " << tid)
    return nullptr;
}
...
int bcast(std::string &str)
{
    ...
    if (dagr_test_util::bcast(str_size))
        return -1;
    char *buf;
    if (rank == 0)
    {
    ...
    if (dagr_test_util::bcast(buf, str_size))
        return -1;
    if (rank != 0)
    {
        str = buf;
