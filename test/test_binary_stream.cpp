...
class dagr_binary_stream_driver
{
public:
    dagr_binary_stream_driver()
    { this->stress_test(); }

    void stress_test();
    void easy_test();

    void fill(dagr_binary_stream &s);

    void validate(dagr_binary_stream &s);
...
int main(int argc, char **argv)
{
    ...
    dagr_binary_stream_driver driver;
    dagr_binary_stream s1, s2;
...
template <typename T>
bool same(p_dagr_variant_array_impl<T> a, p_dagr_variant_array_impl<T> b)
{
    size_t na = a->size();
    size_t nb = b->size();
    if (na != nb)
    {
        DAGR_ERROR("p_dagr_variant_array_impl sizes " << na << " != " << nb)
        return false;
    }
    for (size_t i = 0; i < na; ++i)
...
// --------------------------------------------------------------------------
void dagr_binary_stream_driver::stress_test()
{
    m_random_order = 1;
...
// --------------------------------------------------------------------------
void dagr_binary_stream_driver::easy_test()
{
    m_random_order = 0;
...
// --------------------------------------------------------------------------
int dagr_binary_stream_driver::get_test_case(int i)
{
    if (m_random_order)
...
// --------------------------------------------------------------------------
void dagr_binary_stream_driver::fill(dagr_binary_stream &s)
{
    m_rng.init(177);

#if defined(DAGR_DEBUG)
    vector<int> coverage;
    if (m_random_order)
    ...
    for (int i = 0; i < m_trip_count; ++i)
    {
        ...
        switch (act)
        {
            ...
            {
                p_dagr_variant_array_impl<char> val = dagr_variant_array_impl<char>::New(m_var_array_len);
#ifdef DAGR_DEBUG
            ...
            {
                p_dagr_variant_array_impl<int> val = dagr_variant_array_impl<int>::New(m_var_array_len);
#ifdef DAGR_DEBUG
            ...
            {
                p_dagr_variant_array_impl<float> val = dagr_variant_array_impl<float>::New(m_var_array_len);
#ifdef DAGR_DEBUG
            ...
            {
                p_dagr_variant_array_impl<double> val = dagr_variant_array_impl<double>::New(m_var_array_len);
#ifdef DAGR_DEBUG
            ...
            {
                p_dagr_variant_array_impl<string> val = dagr_variant_array_impl<string>::New(m_var_array_len);
#ifdef DAGR_DEBUG
...
// --------------------------------------------------------------------------
void dagr_binary_stream_driver::validate(dagr_binary_stream &s)
{
    m_rng.init(177);
    ...
    for (int i = 0; i < m_trip_count; ++i)
    {
        ...
        switch (act)
        {
            ...
            case TEST_CHAR_VARIANT_ARRAY:
            {
                p_dagr_variant_array_impl<char> val = dagr_variant_array_impl<char>::New();
                val->from_stream(s);

                p_dagr_variant_array_impl<char> val0 = dagr_variant_array_impl<char>::New(m_var_array_len);
#ifdef DAGR_DEBUG
                cerr << "variant array char = ";
#endif
                for (int j = 0; j < m_var_array_len; ++j)
            ...
            case TEST_INT_VARIANT_ARRAY:
            {
                p_dagr_variant_array_impl<int> val = dagr_variant_array_impl<int>::New();
                val->from_stream(s);

                p_dagr_variant_array_impl<int> val0= dagr_variant_array_impl<int>::New(m_var_array_len);
#ifdef DAGR_DEBUG
                cerr << "variant array int =";
#endif
                for (int j = 0; j < m_var_array_len; ++j)
            ...
            case TEST_FLOAT_VARIANT_ARRAY:
            {
                p_dagr_variant_array_impl<float> val = dagr_variant_array_impl<float>::New();
                val->from_stream(s);

                p_dagr_variant_array_impl<float> val0 = dagr_variant_array_impl<float>::New(m_var_array_len);
#ifdef DAGR_DEBUG
                cerr << "variant array float =";
#endif
                for (int j = 0; j < m_var_array_len; ++j)
            ...
            case TEST_DOUBLE_VARIANT_ARRAY:
            {
                p_dagr_variant_array_impl<double> val = dagr_variant_array_impl<double>::New();
                val->from_stream(s);

                p_dagr_variant_array_impl<double> val0 = dagr_variant_array_impl<double>::New(m_var_array_len);
#ifdef DAGR_DEBUG
                cerr << "variant array double =";
#endif
                for (int j = 0; j < m_var_array_len; ++j)
            ...
            case TEST_STRING_VARIANT_ARRAY:
            {
                p_dagr_variant_array_impl<string> val = dagr_variant_array_impl<string>::New();
                val->from_stream(s);

                p_dagr_variant_array_impl<string> val0 = dagr_variant_array_impl<string>::New(m_var_array_len);
#ifdef DAGR_DEBUG
                cerr << "variant array string =";
#endif
                for (int j = 0; j < m_var_array_len; ++j)
            ...
            case TEST_TABLE:
            {
                ...
                cerr << "table = ";
#endif
                p_dagr_table table0 = dagr_table::New();
                ...
                p_dagr_programmable_algorithm base = dagr_programmable_algorithm::New();
                base->set_number_of_input_connections(0);
                base->set_execute_callback([&](unsigned int,
                        const vector<const_p_dagr_dataset> &,
                        const dagr_metadata &) -> const_p_dagr_dataset
                        { return table0; });

                p_dagr_programmable_algorithm test = dagr_programmable_algorithm::New();
            ...
            case TEST_METADATA:
            {
                ...
                if (!(md  == md0))
                    DAGR_ERROR("tables are different")
            }
            break;
            case TEST_EMPTY_METADATA:
            {
